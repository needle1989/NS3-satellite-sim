...
TraceNextRx (std::string &next_rx_seq_file_name)
{
  ...
  Config::ConnectWithoutContext ("/NodeList/4/$ns3::TcpL4Protocol/SocketList/1/RxBuffer/NextRxSequence", MakeCallback (&NextRxTracer));
}
...
//tracing
   ...
   std::string prefix_file_name = "tc";
     if (tracing)
    {
      std::ofstream ascii;
